  ...
  return (
    <div className="p-4 max-w-2xl mx-auto">
      <h1 className="text-3xl font-bold text-center text-white mb-6">
        <span role="img" aria-label="craving">🍽</span> What Are You Craving?
      </h1>
      ...
      <label className="flex items-center gap-2 mb-6">
        ...
        />
        <span className="text-sm text-white">Use Ingredients From Fridge</span>
      </label>
              ...
              {mealTypes.map((type) => (
                <motion.button
                  ...
                  className={`px-4 py-2 rounded-full border transition ${
                    mealType === type ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-800'
                  }`}
                >
                  {type}
                </motion.button>
            ...
            <div className="flex flex-wrap gap-3">
              {timeOptions.map((t) => (
                <motion.button
                  ...
                  className={`px-4 py-2 rounded-full border transition ${
                    maxReadyTime === t ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-800'
                  }`}
                >
                  Under {t} min
                </motion.button>
            ...
            <div className="flex flex-wrap gap-3">
              {difficultyLevels.map((level) => (
                <motion.button
                  ...
                  className={`px-4 py-2 rounded-full border transition ${
                    difficulty === level ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-800'
                  }`}
                >
                  {level}
                </motion.button>
